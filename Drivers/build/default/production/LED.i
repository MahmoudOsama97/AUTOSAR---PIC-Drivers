...
static void ledsConfigurations(void) {
 ...
 ON_Lamp_Config.port = (Dio_PortType)0;
 ON_Lamp_Config.pin_num = (Dio_ChannelType)2;
 ON_Lamp_Config.direction = PORT_PIN_OUT;
 ON_Lamp_Config.pulling = PULL_DOWN;


 Heater_Lamp_Config.port = (Dio_PortType)0;
 Heater_Lamp_Config.pin_num = (Dio_ChannelType)3;
...
uint8 state = Level ;
  if (state == (1u)) {
   ...
   if (Config_ptr->pulling == PULL_DOWN) {
    Dio_WriteChannel(INDEX_ID,(1u));
   } else if (Config_ptr->pulling == PULL_DOWN) {
    Dio_WriteChannel(INDEX_ID,(0u));
   }
  }
 else if (state == (0u)) {
   if (Config_ptr->pulling == PULL_DOWN) {
